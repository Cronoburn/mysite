...
static private $default 	= array(
	...
	function get_converse_head() {
		...
		if ( wp_is_mobile() ) {
			wp_register_script( 'conversejs', plugins_url( '/core/converse-mobile.min.js', __FILE__ ), array(), self::$default['conver'], true );
			wp_register_style( 'conversejs-mobile', plugins_url( '/core/css/mobile.min.css', __FILE__ ), array('conversejs'), self::$default['conver'] );
			wp_enqueue_style( 'conversejs-mobile' );			
		} else {
			wp_register_script( 'conversejs', plugins_url( '/core/converse-no-jquery.min.js', __FILE__ ), array('jquery'), self::$default['conver'], true );
		}	
		...
		$converse_html = "
			...
			converse.plugins.add('chatme-logout', { 
				...
				visible_toolbar_buttons: { call: {$actual['call']}, clear: {$actual['clear']}, emoticons: {$actual['emoticons']}, toggle_participants: {$actual['toggle_participants']} },
	...
	<form method="post" action="options.php">
    	...
    	<table class="form-table">
        	...
        	<tr valign="top">
        		...
        		<td><input type="checkbox" id="play_sounds" name="play_sounds" value="false" <?php checked('true', get_option('play_sounds')); ?> /> <label for="play_sounds"><?php esc_html_e('Play Sounds', 'conversejs'); ?></label><br/><label for="sounds_path"><?php esc_html_e('Sounds Path', 'conversejs'); ?><input aria-describedby="sounds-description" class="regular-text code" id="sounds_path" name="sounds_path" type="text" placeholder="<?php esc_html_e('./sounds', 'conversejs'); ?>" value="<?php echo get_option('sounds_path'); ?>"></label><p class="description" id="sounds-description"><?php esc_html_e('sound patch ./sounds work with mp3 and odg', 'conversejs'); ?></p></td>
        	</tr>
        	...
        	<td>
        		...
        		<option value="" <?php selected('', get_option('language')); ?>><?php esc_html_e('Some of Site', 'conversejs'); ?></option>
        ...
        <tr valign="top">
        	...
        	<td>
        		...
        		<option value="15" <?php selected('100', get_option('conversejs_priority')); ?>><?php esc_html_e('Hight', 'conversejs'); ?></option>
        		</select>
        	</td>
	</tr>
